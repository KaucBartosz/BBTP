...
BLOCK_SYSTEM_COLOR = [0.9, 0.1, 0.1]  # czerwony – sekwencja systemu
BLOCK_PLAYER_COLOR = [0.1, 0.8, 0.1]  # zielony  – kliknięcia badanego
BLOCK_SIZE = 0.08
GAP = 0.02
RESPONSE_TIMEOUT = 30
...
def main():
    ...
    if keys[0] == "escape":
        win.close()
        if NOUS_LAUNCHER:
            _write_results(SCRIPT_DIR, [], 0, 0, 0, 0, 1, 3, 1)
        return
    ...
    for row in range(grid_size):
        for col in range(grid_size):
            ...
            block = visual.Polygon(
                win,
                edges=60,
                radius=BLOCK_SIZE / 2,
                pos=[x, y],
                fillColor=BLOCK_DEFAULT_COLOR,
                lineColor=[1, 1, 1],
                lineWidth=2,
            )
    ...
    instruction_text = visual.TextStim(
        win, text="Obserwuj sekwencję...", pos=[0, 0.45], height=0.035, color="white"
    )
    feedback_text = visual.TextStim(
        win, text="", pos=[0, -0.45], height=0.04, color="white"
    )
    ...
    while not escaped:
        ...
        instruction_text.text = (
            f"Obserwuj sekwencję... (długość: {current_sequence_length})"
        )
        for seq_idx, block_idx in enumerate(sequence):
            if event.getKeys(["escape"]):
                escaped = True
                break
            grid_blocks[block_idx].fillColor = BLOCK_SYSTEM_COLOR
            for block in grid_blocks:
                block.draw()
        ...
        while not response_complete and not escaped:
            ...
            if is_new_click:
                ...
                for i, (bx, by) in enumerate(block_positions):
                    if point_in_circle(
                        click_pos[0], click_pos[1], bx, by, BLOCK_SIZE / 2
                    ):
                        clicked_block_idx = i
                        break
                ...
                if clicked_block_idx is not None:
                    ...
                    grid_blocks[clicked_block_idx].fillColor = BLOCK_PLAYER_COLOR
                    for block in grid_blocks:
                        block.draw()
                    instruction_text.draw()
                    feedback_text.draw()
                        ...
                        trial_data.append(
                            {
                                ...
                                "sequenceLength": current_sequence_length,
                                "result": "incorrect",
                                "responseTime": clock.getTime() - response_start,
                            }
                        )
                    ...
                    if len(player_sequence) == len(sequence):
                        ...
                        max_correct_length = max(
                            max_correct_length, current_sequence_length
                        )
                        trial_data.append(
                            {
                                "trial": total_trials,
    ...
    avg_rt_ms = (
        round((sum(correct_times) / len(correct_times)) * 1000) if correct_times else 0
    )
    accuracy = round((correct_trials / total_trials) * 100) if total_trials > 0 else 0
